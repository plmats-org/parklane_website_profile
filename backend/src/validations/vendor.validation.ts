...
const majorClientSchema = Joi.object({
  ...
  annual_volume: Joi.string().optional(),
...
const certificationsSchema = Joi.object({
  ...
  quality_control_systems: Joi.string().optional().max(1000),
  environmental_safety_compliance: Joi.array().items(Joi.string()).optional(),
  regulatory_approvals: Joi.array().items(Joi.string()).optional(),
  export_import_licenses: Joi.array().items(Joi.string()).optional(),
...
const sustainabilitySchema = Joi.object({
  ...
  environmental_policy_document: vendorDocumentSchema.optional(),
  waste_management: Joi.string().optional().max(1000),
  ethical_sourcing: Joi.string().valid("yes", "no").optional(),
  ethical_sourcing_details: Joi.string().optional().max(1000),
  social_responsibility: Joi.string().optional().max(1000),
  labor_rights_compliance: Joi.string().valid("yes", "no").optional(),
  labor_rights_document: vendorDocumentSchema.optional(),
});
...
const referencesSchema = Joi.object({
  ...
  international_experience_details: Joi.string().optional().max(1000),
